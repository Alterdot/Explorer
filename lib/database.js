    ...
    if (masternode.proTxHash == null && masternode.txhash == null) {
      ...
      if (masternode.proTxHash != null) {
        ...
        var mn = {
          ...
          lastseen: Math.floor(Date.now() / 1000), // used for removing old nodes that are no longer present in the MN list (see method remove_old_masternodes)
          ip_address: masternode.address.slice(0, -6), // cut out the port
        ...
        var mn = {
          ...
          network: masternode.protocol,
          txhash: masternode.txhash,
          outidx: masternode.outidx,
          status: masternode.status,
          ip_address: masternode.address.slice(0, -6),
          addr: masternode.payee,
          // version: masternode.daemonversion.split('.').join('0'),
          lastseen: masternode.lastseen,
          activetime: masternode.activeseconds,
          lastpaid: masternode.lastpaidtime,
          last_paid_block: masternode.lastpaidblock,
          claim_name: (masternode.claim_name == null ? '' : masternode.claim_name)
        };
      ...
      if (add) {
        var mnToAdd = new Masternode(mn);
        // add new masternode to collection
        mnToAdd.save(function (err) {
          if (err) {
            console.log(err);
            return cb(false);
          } else
        ...
        Masternode.updateOne({ txhash: (masternode.proTxHash != null ? masternode.proTxHash : masternode.txhash) }, mn, function (err) {
          if (err) {
            console.log(err);
            return cb(false);
          } else
